...
for (const queryTest of queryTests) {
    ...
    scenarioQueryUnique.add(testCollection.processDirty(queryTest.label, async (collection) => {
        const item = await collection.queryUnique(queryTest);
        assert("Objects do not match", queryTest.expected, item);
    }));
}
