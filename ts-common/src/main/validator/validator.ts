...
import {currentTimeMillies, Day} from "../utils/date-time-tools";
...
export const validateObjectValues = <V, T = {
    [k: string]: V
}>(validator: ValidatorTypeResolver<V>, mandatory = true): Validator<T> =>
    (path: string, input?: T) => {
        assertMandatory(mandatory, path, input);
        if (!input)
            return;
...
export const isTimestampValid = (time: number, range = {
    min: currentTimeMillies() - 1000 * Day,
    max: currentTimeMillies() + 1000 * Day
}): boolean => {
