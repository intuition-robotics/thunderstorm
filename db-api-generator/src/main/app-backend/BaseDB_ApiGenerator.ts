...
import {ServerApi_Create, ServerApi_Delete, ServerApi_Query, ServerApi_Unique, ServerApi_Update} from "./apis";
...
} from "@intuitionrobotics/thunderstorm/app-backend/modules/FirestoreBackupScheduler";
...
import {FirestoreInterface} from "@intuitionrobotics/firebase/app-backend/firestore/FirestoreInterface";
import {FirestoreTransaction} from "@intuitionrobotics/firebase/app-backend/firestore/FirestoreTransaction";
import {Module} from "@intuitionrobotics/ts-common/core/module";
import {Day} from "@intuitionrobotics/ts-common/utils/date-time-tools";
import {
    BadImplementationException,
    isErrorOfType,
    ThisShouldNotHappenException
} from "@intuitionrobotics/ts-common/core/exceptions";
import {batchAction, filterDuplicates, filterInstances} from "@intuitionrobotics/ts-common/utils/array-tools";
...
// this.setExternalUniqueKeys(["accessLevelIds"]);
    ...
    async createImpl_Read(transaction: FirestoreTransaction, instance: DBType, request?: ExpressRequest): Promise<() => DBType> {
    ...
    async upsert_Read(instance: UType, transaction: FirestoreTransaction, request?: ExpressRequest): Promise<() => DBType> {
        if (instance._id === undefined)
            return this.createImpl_Read(transaction, {
                ...instance,
                _id: generateHex(idLength)
    ...
    protected async upsertAllImpl_Read(instances: UType[], transaction: FirestoreTransaction, request?: ExpressRequest): Promise<(() => DBType)[]> {
        const actions = [] as Promise<() => DBType>[];
    ...
    protected async upsertImpl_Read(transaction: FirestoreTransaction, dbInstance: DBType, request?: ExpressRequest): Promise<() => DBType> {
     ...
     * A promise of the document that was deleted.
     */
    private async deleteImpl_Read(transaction: FirestoreTransaction, ourQuery: {
        where: Clause_Where<DBType>
    }, request?: ExpressRequest): Promise<() => DBType> {
