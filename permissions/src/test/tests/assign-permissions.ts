import {__custom, __scenario, ContextKey, TestException} from "@intuitionrobotics/testelot";
...
import {AssignAppPermissions, PredefinedGroup} from "../../main/shared/apis";
...
async function setPredefinedGroupsPermissions(data: ConfigDB) {
    ...
    const ownerLevel = await AccessLevelPermissionsDB.upsert({
        value: 1000,
        name: "teat-owner-level",
        domainId: domain._id
    });
    const editorLevel = await AccessLevelPermissionsDB.upsert({
        value: 500,
        name: "teat-editor-level",
        domainId: domain._id
    });
    const viewerLevel = await AccessLevelPermissionsDB.upsert({
        value: 200,
    ...
    await GroupPermissionsDB.upsertAll([{...ownerGroup, accessLevelIds: [ownerLevel._id]}, {
        ...editorGroup,
        accessLevelIds: [editorLevel._id]
    }, {...viewerGroup, accessLevelIds: [viewerLevel._id]}]);
}
...
async function isAssignUsersPermissions(data: ConfigDB, assignUserObj: AssignUserModels) {
    ...
    const granterUser = await UserPermissionsDB.upsert({
        accountId: generateHex(32),
        groups: [{
            groupId: GroupPermissionsDB.getPredefinedGroupId(project._id, assignUserObj.granterUserGroup._id),
