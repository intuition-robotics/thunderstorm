...
class ListSingleAdapterBuilder<ItemType= any>
	...
	nested() {
		...
		this.treeNodeRenderer = (props: NodeRendererProps<ItemType>) => {
			...
			const _Renderer = this.renderer
			return <div id={props.node.path} onClick={props.node.onClick}>
				<_Renderer item={typeof props.item === "object" ? (item as unknown as NestedType<ItemType>).item : props.item} node={props.node}/>
			</div>;
		}
...
class TreeSingleAdapterBuilder<RenderItemType= any>
	...
	treeNodeRenderer = (props: NodeRendererProps<RenderItemType>) => {
		...
		const _Renderer = this.renderer
		return <div id={props.node.path} onClick={props.node.onClick}>
			<_Renderer item={typeof props.item === "object" ? (item as unknown as NestedType<RenderItemType>).item : props.item} node={props.node}/>
		</div>;
	}
