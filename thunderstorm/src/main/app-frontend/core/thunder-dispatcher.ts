...
export class ThunderDispatcher<T extends object, K extends FunctionKeys<T>>
    ...
    public static addUIListener(listener: any): void {
        console.info(`Register UI listener: ${listener}`);
        ThunderDispatcher.listenersResolver().push(listener);
    }

    public static removeUIListener(listener: any): void {
        console.info(`Unregister UI listener: ${listener}`);
        removeItemFromArray(ThunderDispatcher.listenersResolver(), listener);
    }
}
